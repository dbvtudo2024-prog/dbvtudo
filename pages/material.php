...
?>
<a href="index.php?p=materiais" class="btn-back-standard">Voltar</a>
<h2>Material</h2>
<?php if ($id <= 0): ?>
...
<?php else: ?>
    <?php
        ...
        $stmt = $pdo->prepare("SELECT * FROM materiais WHERE id = :id LIMIT 1");
        $stmt->execute([':id' => $id]);
        $row = $stmt->fetch();
    ?>
    <?php if (!$row): ?>
        ...
        <div class="card" style="max-width:900px;margin:0 auto;">
            <div class="card-body">
                ...
                <?php if (!empty($row['link'])): ?>
                    <a href="<?php echo htmlspecialchars($row['link']); ?>" class="btn-admin" style="margin-top:10px;" target="_blank" rel="noopener">Abrir link</a>
                <?php endif; ?>
                <?php
                    $viewer = '';
                ...
                <?php elseif (!empty($row['arquivo'])): ?>
                    <a href="uploads/<?php echo $row['arquivo']; ?>" class="btn-admin" style="margin-top:10px;" target="_blank">Abrir arquivo</a>
                <?php endif; ?>
            </div>
        </div>
    <?php endif; ?>
